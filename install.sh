...
# libgit2 does not reliably pick up the ssh-agent, causing
# "no authentication methods succeeded" even when `ssh -T git@github.com` works.
#
# The package name (${BINARY}) is passed as a positional argument because this
# repo is a Cargo workspace with multiple packages; without it cargo errors:
