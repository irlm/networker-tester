...
- [URL Page-Load Diagnostic](#url-page-load-diagnostic)
...
## URL Page-Load Diagnostic

The URL page-load diagnostic is a browser-capable troubleshooting workflow for a real URL.
Unlike the probe modes that target a known `networker-endpoint`, this flow is intended to answer:

- what protocol the page actually used
- how the page loaded from a browser perspective
- what origins/resources were involved
- whether H1/H2/H3 follow-up validation succeeds
- what artifacts were captured (JSON, HAR, PCAP)
...
### Important URL-diagnostic flags

| Flag | Purpose |
|---|---|
| `--url-test-url` | Target page URL (`http` or `https`) |
| `--url-test-auth-token` | Optional bearer token |
| `--url-test-cookie` | Optional cookie header value |
| `--url-test-header` | Extra request header, repeatable (`Name: value`) |
| `--url-test-capture-har` | Generate HAR artifact when supported |
| `--url-test-capture-pcap` | Best-effort packet capture |
| `--url-test-protocol-force` | Restrict follow-up validation probes to `auto`, `h1`, `h2`, or `h3` |
| `--url-test-http3-repeat` | Repeat count for H3 validation probes |
| `--url-test-json` | Print JSON result to stdout |

### What the URL diagnostic currently captures

- browser-visible timings (`dns`, `connect`, `handshake`, `ttfb`, `DOMContentLoaded`, `load`)
- main observed protocol
- per-resource resource timing snapshot
- per-origin summary
- connection summary
- optional protocol validation probe results
- optional HAR artifact
- optional PCAP artifact and structured PCAP summary

### Output artifacts

The URL diagnostic writes a JSON artifact to the configured output directory and may also emit:

- `.har` — browser-style page-load artifact
- `.pcapng` / packet summary files — when packet capture is enabled and available

### Dashboard API endpoints

The API-only dashboard crate currently exposes URL diagnostic data via:

- `GET /api/url-tests`
- `GET /api/url-tests/:run_id`
- `GET /api/url-tests/:run_id/sections`

The `/sections` endpoint is the most dashboard-friendly form because it groups response data into:
- overview
- timings
- protocol
- TLS
