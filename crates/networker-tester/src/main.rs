...
async fn main() -> anyhow::Result<()> {
    ...
    if modes.is_empty() {
        anyhow::bail!(
            "No valid modes specified. Use: tcp,http1,http2,http3,udp,rpm,responsiveness,stamp,mthroughput,ping,path,dualstack,websocket,pmtud,dns,tls,tlsresume,native,curl,\
