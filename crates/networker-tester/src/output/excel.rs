...
///
/// One workbook per run with 8 worksheets:
///   1. Summary        – run metadata
///   2. HTTP Timings   – per-attempt HTTP phase times
///   3. TCP Stats      – kernel-level socket metrics
///   4. TLS Details    – handshake + certificate info
///   5. UDP Stats      – loss / RTT / jitter
///   6. Throughput     – download/upload bandwidth
///   7. Server Timing  – server-side timing from X-Networker-* headers
///   8. Errors         – failed attempts
use crate::metrics::{Protocol, TestRun};
use anyhow::Context;
