...
pub async fn run(client: &Client) -> anyhow::Result<()> {
    ...
    // V036: Unified TestConfig refactor (v0.28.0).
    // See `.critique/refactor/03-spec.md` §1 for the canonical shape.
    //
    // Drops the old `job` + `benchmark_config` + polymorphic `schedule`
    // tables and replaces them with `test_config`, `test_run`,
    // `benchmark_artifact`, and `test_schedule`. Per-protocol phase
    // detail tables (DnsResult/TcpResult/TlsResult/HttpResult/UdpResult)
    // have their `RunId` column renamed to `test_run_id` and re-FK'd
    // against the new `test_run(id)` table. Dev-only — no backfill.
    let row = client
        .query_opt("SELECT version FROM _migrations WHERE version = 36", &[])
        .await?;

    if row.is_none() {
        tracing::info!("Applying V036: unified TestConfig refactor...");
        client.batch_execute(V036_UNIFIED_TEST_CONFIG).await?;
        client
            .execute(
                "INSERT INTO _migrations (version) VALUES (36) ON CONFLICT DO NOTHING",
                &[],
            )
            .await?;
...
/// V036: Unified TestConfig refactor — merges the old `job` (Tests) and
/// `benchmark_config` (Benchmarks) primitives into a single `test_config`
/// table, with an optional `methodology` JSONB that flips a run into
/// benchmark mode.
///
/// Dev-only migration — no backfill. All callers referencing the dropped
/// tables are rewritten in the same PR (Agents B/C/D). See
/// `.critique/refactor/03-spec.md` §1 for the frozen shape.
///
/// CASCADE is used on DROP so FKs from peripheral tables (benchmark_cell,
/// benchmark_regression, project_tester.locked_by_config_id, …) collapse
/// cleanly in one shot. Idempotent via IF EXISTS / IF NOT EXISTS guards.
const V036_UNIFIED_TEST_CONFIG: &str = r#"
