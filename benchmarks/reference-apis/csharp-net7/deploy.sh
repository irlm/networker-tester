#!/usr/bin/env bash
# Deploy the C# .NET 7 reference API to a remote VM.
#
# Usage:
...
REMOTE_DIR="/opt/bench/csharp-net7"
CERT_DIR="/opt/bench"
...
ssh "$TARGET" "sudo mkdir -p $REMOTE_DIR && sudo mkdir -p $CERT_DIR && sudo pkill -f 'csharp-net7' || true"

# Copy published binary
scp -r "$SCRIPT_DIR/publish/"* "$TARGET:/tmp/csharp-net7-deploy/"
ssh "$TARGET" "sudo mv /tmp/csharp-net7-deploy/* $REMOTE_DIR/ && rm -rf /tmp/csharp-net7-deploy"
...
# Start server
ssh "$TARGET" "sudo chmod +x $REMOTE_DIR/csharp-net7 && \
    sudo BENCH_CERT_PATH=$CERT_DIR/cert.pem \
         BENCH_KEY_PATH=$CERT_DIR/key.pem \
         BENCH_PORT=8443 \
         nohup $REMOTE_DIR/csharp-net7 > /var/log/csharp-net7.log 2>&1 &"
