#!/usr/bin/env bash
# Deploy the C# .NET 6 reference API to a remote VM.
#
# Usage:
...
REMOTE_DIR="/opt/bench/csharp-net6"
CERT_DIR="/opt/bench"
...
ssh "$TARGET" "sudo mkdir -p $REMOTE_DIR && sudo mkdir -p $CERT_DIR && sudo pkill -f 'csharp-net6' || true"

# Copy published binary
scp -r "$SCRIPT_DIR/publish/"* "$TARGET:/tmp/csharp-net6-deploy/"
ssh "$TARGET" "sudo mv /tmp/csharp-net6-deploy/* $REMOTE_DIR/ && rm -rf /tmp/csharp-net6-deploy"
...
# Start server
ssh "$TARGET" "sudo chmod +x $REMOTE_DIR/csharp-net6 && \
    sudo BENCH_CERT_DIR=$CERT_DIR \
         BENCH_PORT=8443 \
         nohup $REMOTE_DIR/csharp-net6 > /var/log/csharp-net6.log 2>&1 &"
