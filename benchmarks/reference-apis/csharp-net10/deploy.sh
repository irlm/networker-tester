#!/usr/bin/env bash
# Deploy the C# .NET 10 reference API to a remote VM.
#
# Usage:
...
REMOTE_DIR="/opt/bench/csharp-net10"
CERT_DIR="/opt/bench"
...
ssh "$TARGET" "sudo mkdir -p $REMOTE_DIR && sudo mkdir -p $CERT_DIR && sudo pkill -f 'csharp-net10' || true"

# Copy published binary
scp -r "$SCRIPT_DIR/publish/"* "$TARGET:/tmp/csharp-net10-deploy/"
ssh "$TARGET" "sudo mv /tmp/csharp-net10-deploy/* $REMOTE_DIR/ && rm -rf /tmp/csharp-net10-deploy"
...
# Start server
ssh "$TARGET" "sudo chmod +x $REMOTE_DIR/csharp-net10 && \
    sudo BENCH_CERT_DIR=$CERT_DIR \
         BENCH_PORT=8443 \
         nohup $REMOTE_DIR/csharp-net10 > /var/log/csharp-net10.log 2>&1 &"
