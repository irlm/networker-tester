...
///   <item>If absent/invalid, a random 32-byte dev key is generated so the app
///     still boots (a warning is logged). Unlike the Rust side this key is NOT
///     persisted to disk — a real deployment MUST set the env var, otherwise
///     restarts rotate the key and previously stored secrets become
///     undecryptable. This intentionally mirrors how AuthExtensions handles a
///     missing DASHBOARD_JWT_SECRET.</item>
...
public static class CredentialCipherExtensions
{
    ...
    private static byte[] ResolvePrimaryKey()
    {
        ...
        if (!string.IsNullOrEmpty(hex))
        {
            ...
            catch (Exception ex)
            {
                Console.Error.WriteLine(
                    ...
                    "generating an insecure in-memory dev key instead. Stored cloud " +
                    "credentials will be undecryptable until a valid key is provided.");
            }
        }
        else
        {
            Console.Error.WriteLine(
                ...
                "in-memory dev credential key so the app boots. This key is NOT persisted; " +
                "set a stable 64-hex-char key (openssl rand -hex 32) in production, or " +
                "cloud-account secrets will become undecryptable across restarts.");
        }

        // Dev fallback: random 32-byte key so the process starts (mirrors the
        // JWT dev-secret fallback in AuthExtensions). Not written to disk.
